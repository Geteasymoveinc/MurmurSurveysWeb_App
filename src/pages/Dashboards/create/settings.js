...
class SurveySettings extends Component {
  ...
  componentDidMount() {
    ...
    Geocode.fromAddress(
      ...
      this.setState((state) => ({
        ...
        map: {
          ...state.map,
          marker: { image: Location, position: center, name: location },
        },
      }));
    });
  }
  ...
  render() {
    const { target_audience, active, survey_specific, map } = this.props;
    const { country, city } = map;
    const { location, age, gender, interest, income } = target_audience;
